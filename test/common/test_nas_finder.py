...
def test_nas_ip_available(nas_finder, nas_finder_config):
    assert nas_finder._nas_ip_available(nas_finder_config.ssh_host)
    with pytest.raises(NetworkError):
        assert nas_finder._nas_ip_available('255.255.255.255')


def test_nas_correct(nas_finder, nas_finder_config):
    assert nas_finder._nas_correct(nas_finder_config.ssh_host,
                                   nas_finder_config.ssh_user)


def test_nas_hdd_mounted(nas_finder, nas_finder_config):
    assert nas_finder.nas_hdd_mounted()
