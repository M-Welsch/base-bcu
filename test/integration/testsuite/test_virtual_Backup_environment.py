...
def test_virtual_backup_environment_teardown_old() -> None:
    ...
    for mount_point in [
        test.utils.backup_environment.virtual_hard_drive.VIRTUAL_HARD_DRIVE_MOUNTPOINT,
        environment_directories.SMB_MOUNTPOINT,
    ]:
...
@pytest.mark.skip(reason="deprecated, should be deleted after functional reimplementation")
@pytest.mark.parametrize("protocol, use_vhd", [(Protocol.SSH, False), (Protocol.SSH, True)])
def test_virtual_backup_environment_creation_old(protocol: Protocol, use_vhd: bool) -> None:
    with BackupTestEnvironment(
        protocol=protocol,
        amount_files_in_source=1,
        bytesize_of_each_sourcefile=1024,
