...
def update_sbc_fw():
    return render_template("update_sbc_fw.html", page_name="Update SBC Firmware", user="admin")
...
def upload_and_flash_fw_to_sbc():
    ...
    return render_template(
        "sbc_fw_uploaded_success_message_and_next_steps.html",
...
def setup_backup_hdd_step_1():
    return render_template("setup_backup_hdd_step_1.html", page_name="Setup Backup HDD", user="admin")
...
def setup_backup_hdd_step_2():
    ...
    if not connection_success:
        ...
        return render_template(
            "setup_backup_hdd_step_2.html", page_name="Setup Backup HDD", user="admin", hdd_parameters=hdd_parameters
        )
...
def setup_backup_hdd_step_3():
    ...
    save_hdd_params_to_temp_file(
        hdd_parameters_of_buhdd_to_use, "/tmp/hdd_parameters_of_buhdd_to_use"
    )  # fixme:file is not being created for some reason
    tell_daemon_to_use_new_buhdd()
    return render_template(
        "setup_backup_hdd_step_3.html",
...
def request_hdd_parameters() -> dict:
    [connection_success, hdd_parameters_raw] = send_tcp_message_to_daemon_and_return_answer_or_error(
        "readout_hdd_parameters"
    )
